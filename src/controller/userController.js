  ...
  async getVendorRequest(req, res) {
    try {
      ...
      if (req.query.requestId) {
        vendorRequest = await getVendorRegistrationRquest({ id: req.query.requestId });
      } else if (email === FEMI_EMAIL) vendorRequest = await getVendorRegistrationRquest({ vendorType: 'capex' });
 ...
 */
  async updateVendorRegistrationRequest(req, res) {
    try {
      ...
      if (VendorRegistrationDetails) {
        // eslint-disable-next-line max-len
        emailSent = await sendApprovalRequest(VendorRegistrationDetails, requesterUser.email, email);
      }
